...
try:
    with Schematic(filename = 'pin.svg'):
        ...
        p = Pin(kind='none', name='none', value='none value', w=4)
        Wire([shift_x(p.C, -50), shift_x(p.C, 50)])
        p = Pin(kind='dot', name='dot', C=p.C, yoff=50, value='dot value', w=4)
        Wire([shift_x(p.C, -50), shift_x(p.C, 50)])
        Wire([shift_y(p.C, -25), shift_y(p.C, 25)])
        p = Pin(kind='in', name='in', C=p.C, yoff=50, w=4)
        Wire([p.C, shift_x(p.C, 25)])
        p = Pin(kind='out', name='out', C=p.C, yoff=50, w=4)
